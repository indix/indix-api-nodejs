...
var VERSION = _config2.default.version;
console.log(HOST);
console.log(VERSION);
...
function getJobStatus(jobId) {
  ...
  var endpoint = '/' + VERSION + '/bulk/jobs/' + jobId + '?app_id=' + appID + '&app_key=' + appKey;
  var url = HOST + endpoint;
  return new _promise2.default(function (fulfill, reject) {
    (0, _request2.default)(url, function (error, response, body) {
      if (!error && response.statusCode == 200) {
...
function downloadProducts(jobID) {
  ...
  var url = 'https://api.indix.com/' + VERSION + '/bulk/jobs/' + jobID + '/download?app_id=' + appID + '&app_key=' + appKey;
